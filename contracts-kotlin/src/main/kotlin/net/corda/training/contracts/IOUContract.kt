...
/**
 * This is where you'll add the contracts code which defines how the [IOUState] behaves. Look at the unit tests in
...
class IOUContract : Contract {
    ...
    /**
     * Add any commands required for this contracts as classes within this interface.
    ...
    override fun verify(tx: LedgerTransaction) {
        ...
        when (command.value) {
            is Commands.Issue -> requireThat {
                ...
                "Only one output states should be created when issuing an IOU." using (tx.outputs.size == 1)
            ...
            is Commands.Transfer -> requireThat {
                "An IOU transfer transaction should only consume one input states." using (tx.inputs.size == 1)
                "An IOU transfer transaction should only create one output states." using (tx.outputs.size == 1)
